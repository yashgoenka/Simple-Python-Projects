...
def is_swap(score0, score1):
    ...
    # BEGIN PROBLEM 5
    "*** REPLACE THIS LINE ***"
    if (min(score0,score1)*2)==max(score0,score1):
        return True
    else:
        return False
    # END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    iter_result=0
    for i in range(1,11):
        item_result = make_averaged(roll_dice)(i, dice)
        if iter_result < item_result:
            iter_result = item_result
            max_iter = i
    return max_iter
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score + bacon_score, opponent_score):
        if opponent_score>(score+bacon_score):
            return 0
        else:
            return num_rolls
